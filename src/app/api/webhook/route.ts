...
export async function POST(req: NextRequest){
    ...
    if (!signature || !apiKey){
        return NextResponse.json(
            { error: "Missing signautre or API key" },
            { status: 400 }
        )
    }
    ...
    if (eventType === "call.session_started"){
        ...
        realtimeClient.updateSession({
            instructions: existingAgent.instructions || "You are a helpful assistant that can answer questions and help with tasks.",
        });
        ...
        if (userId !== existingAgent.id){
            const instructions = `
                ...
                const previousMessages = channel.state.messages.slice(-5).filter((msg) => msg.text && msg.text.trim() !== "").map<ChatCompletionMessageParam>((msg) => ({
